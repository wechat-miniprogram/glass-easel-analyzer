use glass_easel_template_compiler::parse::{
    tag::{Element, ElementKind, Ident, Node, StaticAttribute, TemplateDefinition, Value},
    Position, Template, TemplateStructure,
};

use crate::{
    utils::{add_file_extension, location_to_lsp_range},
    wxml_utils::{
        for_each_scope_ref, for_each_scope_ref_in_subtree, for_each_slot,
        for_each_static_class_name_in_element, for_each_template_element, insert_element_scopes,
        ScopeKind, Token,
    },
};
...
pub(super) fn find_elements_matching_id(
    ...
    rec_import_and_include_elements(project, abs_path, template, |abs_path, elem| {
        if let ElementKind::Normal { common, .. } = &elem.kind {
            let elem_id_loc = common.id.as_ref().and_then(|x| match &x.1 {
                Value::Static {
                    value, location, ..
                } => Some((value.as_str(), location)),
                _ => None,
            });
...
fn find_slot_matching<'a>(
    ...
    for_each_slot(template, |slot_elem| match &slot_elem.kind {
    ...
    for import in source_tamplate.globals.imports.iter().rev() {
        if let Some(p) = project.find_rel_path_for_file(abs_path, &import.src.name) {
            ...
            if let Ok(imported_template) = project.get_wxml_tree(&imported_path) {
                if let Some(x) = imported_template
                    .globals
                    .sub_templates
                    .iter()
                    .rfind(|x| x.name.is(is))
                {
...
fn search_wxml_template_usages(
    ...
    for (source_p, tree) in project.list_wxml_trees() {
        let Ok(source_p) = crate::utils::join_unix_rel_path(root, source_p, root) else {
            continue;
        };
        if let Some((p, _)) = get_target_template_path(project, &source_p, tree, is) {
            if p.as_path() != abs_path {
                continue;
            };
            crate::wxml_utils::for_each_template_element(tree, |elem, _| match &elem.kind {
                ElementKind::TemplateRef { target, .. } => match &target.1 {
                    Value::Static {
                        value, location, ..
                    } => {
                        if value.as_str() == is {
...
pub(super) fn find_declaration(
    ...
    if let Ok(template) = project.get_wxml_tree(abs_path) {
        ...
        match token {
            ...
            Token::ScopeRef(loc, kind) => match kind {
                ...
                ScopeKind::SlotValue(attr, parent) => {
                    ...
                    if to_definition && parent_tag_name.is_some() {
                        ...
                        if let Some((target_wxml_path, ranges)) =
                            find_slot_matching(&project, abs_path, tag_name_str, &attr.name)
                        {
                            for target_loc in ranges {
                                ret.push(LocationLink {
                                    origin_selection_range: Some(location_to_lsp_range(&loc)),
                                    target_uri: lsp_types::Url::from_file_path(&target_wxml_path)
                                        .unwrap(),
            ...
            Token::SlotValueScope(name, key, parent) => {
                ...
                if to_definition && parent_tag_name.is_some() {
                    let tag_name_str: &str = &parent_tag_name.unwrap().name;
                    if let Some((target_wxml_path, ranges)) =
                        find_slot_matching(&project, abs_path, tag_name_str, &key)
                    {
                        for target_loc in ranges {
                            ret.push(LocationLink {
                                origin_selection_range: Some(location_to_lsp_range(&name.location)),
            ...
            Token::TemplateRef(is, loc) => {
                    ...
                    .find(|x| x.name.is(is))
                {
                    ...
                    if let Some((imported_path, def)) =
                        get_target_template_path(project, abs_path, template, is)
                    {
                        ret.push(LocationLink {
                            origin_selection_range: Some(location_to_lsp_range(&loc)),
...
pub(super) fn find_references(
    ...
    let mut ret: Vec<Location> = find_declaration(project, abs_path, pos, true)
        ...
        .map(|x| Location {
            uri: x.target_uri,
            range: x.target_selection_range,
        })
        .collect();
    if let Ok(template) = project.get_wxml_tree(abs_path) {
        let token = crate::wxml_utils::find_token_in_position(
        ...
        match token {
            Token::TagName(ident) => {
                project.search_component_wxml_usages(
                    abs_path,
                    &ident.name,
                    |source_wxml, template, expected_tag_name| {
            ...
            Token::ForItem(name, elem) | Token::ForIndex(name, elem) => {
                for_each_scope_ref(template, |loc, kind| match kind {
            ...
            | Token::SlotValueRefAndScope(key, _, parent) => {
                ...
                if let Some(tag_name) = parent_tag_name {
                    ...
                    project.search_component_wxml_usages(
                        ...
                        &tag_name.name,
                        |source_wxml, template, expected_tag_name| {
                            crate::wxml_utils::for_each_template_element(
                                template,
                                |elem, parent_scopes| {
                                    ...
                                    for child in children {
                                        let Node::Element(child_elem) = child else {
                                            continue;
                                        };
                                        let Some(mut refs) = child_elem.slot_value_refs() else {
                                            continue;
                                        };
                                        let Some(expected_attr) =
                                            refs.find(|attr| attr.name.name == expected_key_name)
                                        else {
                                            continue;
                                        };
                                        ret.push(Location {
                                            uri: lsp_types::Url::from_file_path(&source_wxml)
                                                .unwrap(),
                                            range: location_to_lsp_range(
                                                &expected_attr.name.location,
                                            ),
                                        });
                                        let mut new_scopes =
                                            parent_scopes.iter().cloned().collect();
                                        insert_element_scopes(&mut new_scopes, elem);
                                        for_each_scope_ref_in_subtree(
                                            child,
                                            &mut new_scopes,
                                            |loc, kind| {
