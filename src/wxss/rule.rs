...
impl CSSParse for Selector {
    fn css_parse(ps: &mut ParseState) -> Option<Self> {
        fn collect_unknown(ps: &mut ParseState) -> Option<Selector> {
            ...
            while let Some(peek) = ps.peek() {
                match peek {
                    TokenTree::Comma(_) | TokenTree::Semicolon(_) | TokenTree::Brace(_) => break,
                    _ => v.push(ps.next()?),
                }
            }
            if v.is_empty() {
        ...
        let ret = match ps.peek()? {
            TokenTree::Semicolon(_) | TokenTree::Brace(_) => return None,
            ...
            TokenTree::Colon(_) => {
                ...
                if let Some(peek2) = ps.peek_with_whitespace() {
                    if peek2.is_ident_or_function() {
                        ...
                        let op2 = CSSParse::css_parse(ps)?;
                        if ps
                            .peek_with_whitespace()
                            .is_some_and(|x| x.is_ident_or_function())
                        {
    ...
    fn location(&self) -> Location {
        match self {
            ...
            Self::Class(op, x) => op.location().start..x.location().end,
...
impl CSSParse for IdentOrFunction {
    fn css_parse(ps: &mut ParseState) -> Option<Self> {
        let ret = match ps.peek()? {
            ...
            TokenTree::Function(_) => {
                Self::Function(ps.parse_function(|ps| Some(ps.skip_to_end()))?)
            }
            _ => return None,
        };
        Some(ret)
