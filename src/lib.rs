...
fn handle_request(Request { id, method, params }: Request) -> anyhow::Result<Response> {
    macro_rules! handler {
        ($name:expr, $f:path) => {
            if method.as_str() == $name {
                let params = serde_json::from_value(params)
                    .map_err(|err| anyhow::Error::from(err).context(format!("Invalid params on {:?}", method)))?;
                let ret = futures::executor::block_on($f(params))?;
...
fn handle_notification(Notification { method, params }: Notification) -> anyhow::Result<()> {
    macro_rules! handler {
        ($name:expr, $f:path) => {
            if method.as_str() == $name {
                let params = serde_json::from_value(params)
                    .map_err(|err| anyhow::Error::from(err).context(format!("Invalid params on {:?}", method)))?;
                return Ok(futures::executor::block_on($f(params))?);
            }
        };
    }
...
struct InitializeParams {
    #[serde(default, rename = "backendConfiguration")]
        ...
        match msg {
            Message::Request(req) => {
                match handle_request(req) {
                    Ok(res) => {
                        if let Err(err) = sender.send(Message::Response(res)) {
                            log::error!("{}", err);
                        }
                    }
                    Err(err) => {
            ...
            Message::Notification(note) => {
                if let Err(err) = handle_notification(note) {
                    log::error!("{}", err);
                }
            }
        }
